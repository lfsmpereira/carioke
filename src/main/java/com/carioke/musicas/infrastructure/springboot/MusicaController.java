...
@RestController
@RequestMapping(path = RestApiRoutes.Musicas.MUSICAS)
public class MusicaController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity incluir(@RequestBody @Valid MusicaPostDto dados) {
        List<Compositor> compositores = dados.compositores_ids().stream().map(a->(compositorRepository.getReferenceById(a))).toList();
        repository.save(new Musica(dados.titulo(), compositores));
